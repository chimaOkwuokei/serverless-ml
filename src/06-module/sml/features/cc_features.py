...
def card_owner_age(trans_df: pd.DataFrame, profiles_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate age in years using days, then divide by 365.25 to account for leap years
    age_df["age_at_transaction"] = (
        (age_df["datetime"] - age_df["birthdate"]).dt.total_seconds() / (365.25 * 24 * 60 * 60)
    )
    
    profiles_df = age_df[["name", "sex", "mail", "birthdate", "City", "Country", "cc_num", "datetime", "month"]]
    return age_df, profiles_df
